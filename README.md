...
![UI Preview](./png/ui-reference.png)

## プロジェクト概要
スライド式の数字・画像パズルをオフラインでも遊べるように最適化し、Gemini 2.5 Flash & Imagen 4.0 を活用してヒント提示・最適解生成・プレイスタイル分析まで行う学習型アプリケーションです。画像アップロードやAI生成を通して常に新鮮な盤面を提供し、家族でもソロでも楽しめるように設計されています。

## 主な特徴
### 画像ソースの自由度
- ローカル画像アップロード（JPEG/PNG/GIF）に加え、Imagen 4.0 によるテキスト→画像生成をサポート
...
- Google Generative AI（Gemini 2.5 Flash, Imagen 4.0）
- next-pwa, Workbox, Web Audio API, Canvas API

```
app/
├─ actions/ai.ts            # Gemini ヒント・解析 Server Actions
├─ actions/image.ts         # Imagen 画像生成 Server Actions
...
2. 依存関係をインストール  
   ...
   npm install
   ```
3. `./.env.local` を作成して Gemini / Imagen 用 API キーを設定  
   ```env
...
## デザインアセット
- `png/ui-reference.png`: README / Issue テンプレで使える UI 俯瞰画像
...
## ロードマップのヒント
- AI キー自動検証や Imagen と Gemini の個別レート制限管理
